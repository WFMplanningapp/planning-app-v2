    ...
    switch (action) {
      ...
      case 'REMOVE':
        if (
          ...
          data.lobs.find((lob) => lob.project === selection.get('project')._id)
        ) {
          alert(
            'There are still one or more Lob/s associated with this Project. Please delete them before proceeding'
          );
        } else {
          await fetch(
            `/api/data/management/project?id=${
