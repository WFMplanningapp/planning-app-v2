...
function isHoursModel(model) {
  return normalizeModel(model) === "hours";
}
...
// ================================================================
// SHRINKAGE (Convert HOURS from Gross to Produtive and vice-versa)
// ================================================================
...
function calculateIntervalHours(
  ...
  const safeDemandHours = Math.max(
    0,
    toFiniteNumber(demandHours, 0)
  );

  const safeIntervalHours = toPositiveNumber(
    intervalHours,
    0.5
  );

  const normalizedBasis = normalizeModel(
    channelConfig?.hoursBasis
  );
  ...
  const internalShrinkFraction = Math.min(
    Math.max(
      0,
      toFiniteNumber(internalShrinkPct, 0)
    ) / 100,
    0.95
  );

  const externalShrinkFraction = Math.min(
    Math.max(
      0,
      toFiniteNumber(externalShrinkPct, 0)
    ) / 100,
    0.95
  );

  const inputAgents =
    safeDemandHours / safeIntervalHours;

  let productiveAgents;
  ...
  if (basis === "gross") {
    productiveAgents =
      ...
      (1 - externalShrinkFraction) *
      (1 - internalShrinkFraction);
  } else if (basis === "incenter") {
    productiveAgents =
      inputAgents *
      (1 - internalShrinkFraction);
  } else {
    productiveAgents = inputAgents;
  }

  return {
    productive: productiveAgents,
    occupancy: null,
    limitingConstraint: "hours",
    hoursBasis: basis,
    inputHours: safeDemandHours,
    inputAgents,
  };
}
...
export function calculateChannelDay({
  ...
  const modelIsHours = isHoursModel(
  channelConfig?.model
  );
