...
export const generateCapacity = (capPlan, entries, weeks, fromWeek) => {
  ...
  let newPlan = weeks.map((week) => {
    ...
    if (entry && entry.attrition) {
      ...
      if (newPlanWeek.expectedFTE >= 0 || newPlanWeek.actualFTE !==0) 
    ...
    // console.log('Week', newPlanWeek );
     // console.log('Outra Merda qq', current);
    // console.log('expectedFTE', newPlanWeek.expectedFTE);
    // console.log('expectedAttrition', newPlanWeek.expectedAttrition);
    // console.log('moveIn', newPlanWeek.moveIn);
    // console.log('moveOut', newPlanWeek.moveOut);
    // console.log('nesting', current.nesting);
    ...
    current.inTraining.forEach((batch) => {
      ...
      if (batch.weeksToLive > 1) {
        ...
        newPlanWeek.actualFTE += trainingTotal - batch.ocpAttrition
        ...
        if (newPlanWeek.expectedFTE !== undefined) {
          if (current.fcTrAttrition && current.isFuture) {
            ...
            newPlanWeek.expectedFTE += trainingTotal - batch.ocpAttrition 
    ...
    if (entry && entry.actVac) { 
      ...
      newPlanWeek.actualFTE = newPlanWeek.actualFTE - (newPlanWeekactualFTENoActuals * (actOffPercents / 100))
    }
    if (entry && entry.actAbs) {
      const actAbsPercents = Number(entry.actAbs)
      newPlanWeek.actualFTE = newPlanWeek.actualFTE - (newPlanWeekactualFTENoActuals * (actAbsPercents / 100))
    }
    ...
    if (entry && entry.plannedVac) {
      ...
      newPlanWeek.expectedFTE = newPlanWeek.expectedFTE - (newPlanWeekExpectedFTENoPlanned * (plannedVacPercents / 100) )
    }
    if (entry && entry.plannedAbs) {
      const plannedAbsPercents = Number(entry.plannedAbs)
      newPlanWeek.expectedFTE = newPlanWeek.expectedFTE - (newPlanWeekExpectedFTENoPlanned * (plannedAbsPercents / 100))
    }
    ...
    current.billableFTE &&
      (newPlanWeek.expectedFTE
        ...
        : (newPlanWeek.billVar =
            newPlanWeek.actualFTE - newPlanWeek.billableFTE))
    ...
    current.fcFTE &&
      (newPlanWeek.expectedFTE
        ...
        : (newPlanWeek.fcVar = newPlanWeek.actualFTE - newPlanWeek.fcFTE))

    //current = { ...current, ...newPlanWeek, actualFTE : newPlanWeek.totalHC, expectedFTE : current.isFuture ? newPlanWeek.actualFTE : null}
    current = { ...current, ...newPlanWeek, actualFTE: newPlanWeekactualFTENoActuals, expectedFTE: newPlanWeekExpectedFTENoPlanned}
