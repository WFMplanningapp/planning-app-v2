...
export const generateCapacity = (capPlan, entries, weeks, fromWeek) => {
  ...
  current.productiveRequirement = parseInt(current.entry.productiveRequirement);
  ...
  let newPlan = weeks.map((week) => {
    ...
    const totalHCNoOvertime = newPlanWeek.totalHC;
    const totalFTENoOvertime = newPlanWeek.totalFTE;
    const actualFTENoOvertime = newPlanWeek.actualFTE;
    const actProdFTENoOvertime = newPlanWeek.ActProdFTE;
    const expectedFTENoOvertime = newPlanWeek.expectedFTE;
    const planProdFTENoOvertime = newPlanWeek.PlanProdFTE;
    ...
    current.productiveRequirement &&
      (newPlanWeek.expectedFTE
        ? (newPlanWeek.billVar =
            newPlanWeek.PlanProdFTE - newPlanWeek.productiveRequirement)
        : (newPlanWeek.billVar =
            newPlanWeek.ActProdFTE - newPlanWeek.productiveRequiremen));

    current.inCenterRequirement &&
      (newPlanWeek.expectedFTE
        ? (newPlanWeek.reqVar =
            newPlanWeek.expectedFTE - newPlanWeek.inCenterRequirement)
        : (newPlanWeek.reqVar =
            newPlanWeek.actualFTE - newPlanWeek.inCenterRequirement));
    ...
    current = {
      ...current,
      ...newPlanWeek,
      totalHC: totalHCNoOvertime,
      totalFTE: totalFTENoOvertime,
      actualFTE: actualFTENoOvertime,
      ActProdFTE: actProdFTENoOvertime,
      expectedFTE: expectedFTENoOvertime,
      PlanProdFTE: planProdFTENoOvertime,
...
// Determine if this week is future, present, or past:
    const today = new Date();
    const weekStart =
      week.firstDate instanceof Date ? week.firstDate : new Date(week.firstDate);
    const weekEnd = new Date(weekStart.getTime() + 7 * 24 * 60 * 60 * 1000);
    const isFutureOrCurrent = weekStart <= today && today < weekEnd || weekStart > today; // update logic if you want "today" to count as past or not

// Composite InCenter FTE: Use planned for future, actualInCenterFTE for past (or fallback)
// Use actualFTE for past, expectedFTE (planned) for present/future
  newPlanWeek.compositeInCenterFTE = isFutureOrCurrent
    ? newPlanWeek.expectedFTE
    : newPlanWeek.actualFTE ?? newPlanWeek.expectedFTE;

  // Use ActProdFTE for past, PlanProdFTE for present/future
  newPlanWeek.compositeProductiveFTE = isFutureOrCurrent
    ? newPlanWeek.PlanProdFTE
