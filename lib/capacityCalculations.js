...
export const generateCapacity = (capPlan, entries, weeks, fromWeek) => {
  ...
  let newPlan = weeks.map((week) => {
    ...
    if (isOverridden && hasManualGross) {
      ...
      newPlanWeek.reqSource = "manual";
    } else if (hasEngineGross) {
      // Engine value available — use it
      newPlanWeek.grossRequirement = parseFloat(entry.engineGrossReq);
      lastEngineGrossReq = newPlanWeek.grossRequirement;
      newPlanWeek.reqSource = "engine";
    } else if (hasManualGross) {
      // No engine value, use manual (legacy behavior)
      newPlanWeek.grossRequirement = parseFloat(entry.grossRequirement);
      newPlanWeek.reqSource = "manual";
    } else if (lastEngineGrossReq !== null) {
      // Carry forward last engine value
      newPlanWeek.grossRequirement = lastEngineGrossReq;
      newPlanWeek.reqSource = "engine-carried";
    } else {
      // Carry forward last manual value (original behavior)
      newPlanWeek.grossRequirement = current.grossRequirement;
      newPlanWeek.reqSource = "carried";
    }
    ...
    if (isOverridden && hasManualInCenter) {
      newPlanWeek.inCenterRequirement = parseFloat(entry.inCenterRequirement);
    } else if (hasEngineInCenter) {
      newPlanWeek.inCenterRequirement = parseFloat(entry.engineInCenterReq);
      lastEngineInCenterReq = newPlanWeek.inCenterRequirement;
      lastInCenterRequirement = newPlanWeek.inCenterRequirement;
    } else if (hasManualInCenter) {
      newPlanWeek.inCenterRequirement = parseFloat(entry.inCenterRequirement);
      lastInCenterRequirement = newPlanWeek.inCenterRequirement;
    } else if (lastEngineInCenterReq !== null) {
      newPlanWeek.inCenterRequirement = lastEngineInCenterReq;
    } else {
    ...
    if (isOverridden && hasManualProductive) {
      newPlanWeek.productiveRequirement = parseFloat(entry.productiveRequirement);
    } else if (hasEngineProductive) {
      newPlanWeek.productiveRequirement = parseFloat(entry.engineProductiveReq);
      lastEngineProductiveReq = newPlanWeek.productiveRequirement;
      lastProductiveRequirement = newPlanWeek.productiveRequirement;
    } else if (hasManualProductive) {
      newPlanWeek.productiveRequirement = parseFloat(entry.productiveRequirement);
      lastProductiveRequirement = newPlanWeek.productiveRequirement;
    } else if (lastEngineProductiveReq !== null) {
      newPlanWeek.productiveRequirement = lastEngineProductiveReq;
    } else {
    ...
    return {
      ...
      // Preserve the effective requirement values selected above.
      // These may come from an override, the engine, a legacy manual
      // value, or carry-forward logic.
      grossRequirement: newPlanWeek.grossRequirement,
      inCenterRequirement: newPlanWeek.inCenterRequirement,
      productiveRequirement: newPlanWeek.productiveRequirement,

      reqSource: newPlanWeek.reqSource,
      engineOverride: newPlanWeek.engineOverride,
