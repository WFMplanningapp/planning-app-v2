...
export const generateCapacity = (capPlan, entries, weeks, fromWeek) => {
  ...
  let newPlan = weeks.map((week) => {
    ...
    if (entry && entry.overtimeFTE) {
      ...
      const overtimeValue = parseFloat(entry.overtimeFTE);

      const originalTotalHC = newPlanWeek.totalHC;
      const originalActualFTE = newPlanWeek.actualFTE;
      const originalActProdFTE = newPlanWeek.ActProdFTE;     
      const originalTotalFTE = newPlanWeek.totalFTE;
      const originalExpectedFTE = newPlanWeek.expectedFTE;
      const originalPlanProdFTE = newPlanWeek.PlanProdFTE;


      newPlanWeek.totalHC += overtimeValue;
      newPlanWeek.actualFTE += overtimeValue;
      newPlanWeek.ActProdFTE += overtimeValue;     
      newPlanWeek.totalFTE += overtimeValue;
      newPlanWeek.expectedFTE += overtimeValue;
      newPlanWeek.PlanProdFTE += overtimeValue;

      // Optionally, log the original and updated values
    console.log(`Original Values: TotalHC: ${originalTotalHC}, ActualFTE: ${originalActualFTE}, ActProdFTE: ${originalActProdFTE}, TotalFTE: ${originalTotalFTE}, ExpectedFTE: ${originalExpectedFTE}, PlanProdFTE: ${originalPlanProdFTE}`);
    console.log(`Updated Values after Overtime: TotalHC: ${newPlanWeek.totalHC}, ActualFTE: ${newPlanWeek.actualFTE}, ActProdFTE: ${newPlanWeek.ActProdFTE}, TotalFTE: ${newPlanWeek.totalFTE}, ExpectedFTE: ${newPlanWeek.expectedFTE}, PlanProdFTE: ${newPlanWeek.PlanProdFTE}`);

    current.originalValues = {
      totalHC: originalTotalHC,
      actualFTE: originalActualFTE,
      actProdFTE: originalActProdFTE,
      totalFTE: originalTotalFTE,
      expectedFTE: originalExpectedFTE,
      planProdFTE: originalPlanProdFTE,
    };

   }

   // Calculate billVar based on whether the week is past or future
   if (current.billableFTE) {
    if (current.isFuture) {
      // For future weeks, calculate reqVar as the difference between expected FTE and required FTE
      newPlanWeek.billVar = newPlanWeek.PlanProdFTE - newPlanWeek.billableFTE;
    } else {
      // For past weeks, calculate reqVar as the difference between actual FTE and required FTE
      newPlanWeek.billVar = newPlanWeek.ActProdFTE - newPlanWeek.billableFTE;
    }
  }


  // Calculate reqVar based on whether the week is past or future
  if (current.requiredFTE) {
    if (current.isFuture) {
      // For future weeks, calculate reqVar as the difference between expected FTE and required FTE
      newPlanWeek.reqVar = newPlanWeek.expectedFTE - newPlanWeek.requiredFTE;
    } else {
      // For past weeks, calculate reqVar as the difference between actual FTE and required FTE
      newPlanWeek.reqVar = newPlanWeek.actualFTE - newPlanWeek.requiredFTE;
    }
  }

  
  // Calculate burgetVAR based on whether the week is past or future
  if (current.totalFTE) {
    if (current.isFuture) {
      // For future weeks, calculate reqVar as the difference between expected FTE and required FTE
      newPlanWeek.fcVar = newPlanWeek.totalFTE - newPlanWeek.budgetFTE;
