...
export const generateCapacity = (capPlan, entries, weeks, fromWeek) => {
  ...
  let newPlan = weeks.map((week) => {
    ...
    if (entry && entry.actVac) { 
      const actOffPercents = Number(entry.actVac.split("%")[0])
      newPlanWeek.totalFTE = newPlanWeek.totalFTE - (newPlanWeek.totalHC * (actOffPercents / 100))
    }
    if (entry && entry.actAbs) {
      const actAbsPercents = Number(entry.actAbs.split("%")[0])
      newPlanWeek.totalFTE = newPlanWeek.totalFTE - (newPlanWeek.totalHC * (actAbsPercents / 100))
      console.log(current.firstDate)
    }
    console.log(newPlanWeek.firstDate, current.firstDate)

    if (entry && entry.plannedVac) {
      const plannedVacPercents = Number(entry.plannedVac.split("%")[0])
      newPlanWeek.expectedFTE = newPlanWeek.expectedFTE - (newPlanWeek.totalHC * (plannedVacPercents / 100) )
    }

    if (entry && entry.plannedAbs) {
      const plannedAbsPercents = Number(entry.plannedAbs.split("%")[0])
      newPlanWeek.expectedFTE = newPlanWeek.expectedFTE - (newPlanWeek.totalHC * (plannedAbsPercents / 100))
    }
