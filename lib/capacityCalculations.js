...
export const generateCapacity = (capPlan, entries, weeks, fromWeek) => {
  ...
  let newPlan = weeks.map((week) => {
    ...
    // We store the newPlanWeek totalFTE in order to reset it to this value for the next iteration
    const newPlanWeekTotalFTENoActuals = newPlanWeek.totalFTE;
    if (entry && entry.actVac) { 
      const actOffPercents = Number(entry.actVac)
      newPlanWeek.totalFTE = newPlanWeek.totalFTE - (newPlanWeekTotalFTENoActuals * (actOffPercents / 100))
    }
    if (entry && entry.actAbs) {
      const actAbsPercents = Number(entry.actAbs)
      newPlanWeek.totalFTE = newPlanWeek.totalFTE - (newPlanWeekTotalFTENoActuals * (actAbsPercents / 100))
    }
    ...
    //current = { ...current, ...newPlanWeek, totalFTE : newPlanWeek.totalHC, expectedFTE : current.isFuture ? newPlanWeek.totalFTE : null}
    current = { ...current, ...newPlanWeek, totalFTE: newPlanWeekTotalFTENoActuals, expectedFTE: newPlanWeekExpectedFTENoPlanned}
